...
Preliminarmente, se definen:

### Espacio N-Dimensional
Sea $\mathbb{R}^{n}$ el espacio n-dimensional, dotado de la [[Norma Euclidiana|norma euclidiana]]:
$$
\lvert \lvert \vec{x} \rvert  \rvert_{2} =\sqrt{ x_{1}^{2}+x_{2}^{2}+\dots+x_{n}^{2} }=\sqrt{ \vec{x}\cdot \vec{x} }
$$
*obs:* Se denota la **posición** como $\vec{x}$ o $\vec{r}$ en $\mathbb{R}^{3}$

### [[Campo Escalar]]
***def:*** Sea $\Omega \subseteq \mathbb{R}^{3}$ abierto y no vacío,
...
*ejemplo:* los puntos de un disco plano que gira en sentido antihorario, con velocidad angular $\omega>0$ constante y de radio $r=1$, tiene como campos de velocidad:
![[Pasted image 20250828111223.png#invert_B]]

$$
\implies  \vec{v}(x,y,0)=-\omega y \hat{i} + \omega x \hat{j}+0\hat{k}
$$
*Ejemplo 2*: Gravitación universal
*Ejemplo 3*: Gravitación cerca de la tierra

## Integración en Curvas N-Dimensionales
Para integrar en $\mathbb{R}^n$ se requiere un conjunto [[Jordan-medible]]. *Esto es equivalente a decir que el 'borde' tenga medida 0*, como por ejemplo: el volumen

Se va a integrar una curva, pero una curva no tiene volumen, entonces, se define para poder continuar.

### [[Curva Continua]]
***def:*** Una **curva continua** en $\mathbb{R}^n$ es un par $(\Gamma,\vec{\gamma})$, donde:
$$
\Gamma=\vec{\gamma}([a,b])
$$

y $\vec{\gamma}:[a,b]\to \mathbb{R}^n$ se llama [[parametrización]] de $\Gamma$, donde
$$
...
![[Pasted image 20250828111843.png#invert_B]]
...
### [[Traza]]
***def:*** Diremos que un punto $x_{0}$ está sobre $\Gamma$ si existe 
$$
t_{0} \in [a,b]: \vec{x_{0}}=\vec{\gamma}(t_{0})
$$
...
*obs:* Para el conjunto $[a,b]$
![[Pasted image 20250828112103.png#invert_B]]
$\vec{\gamma}'(a),\;\vec{\gamma}'(b)$ deben ser interpretadas como derivadas a la izquierda y derecha, respectivamente.

***def:*** Una curva es **suave (o de [[clase C1]])** si su parametrización $\vec{\gamma}:[a,b]\to \mathbb{R}^{n}$ es continuamente diferenciable, y además:
$$
...
![[Pasted image 20250828112140.png#invert_B]]
Si $\vec{\gamma_{1}}:[a_{1},b_{1}],\; \vec{\gamma_{2}}:[a_{2},b_{2}]$ son parametrizaciones de $\Gamma_{1},\Gamma_{2}$ respectivamente, son clase $\mathcal{C}^{1}$ por tramos y cumplen que $\vec{\gamma}_{1}(b_{1})=\vec{\gamma}_{2}(a_{2})$ (coinciden en un punto), entonces:
$$
...
\right.
$$
es una parametrización de la curva $\Gamma$, que es la suma de ambas curvas $\Gamma_{1},\Gamma_{2}$ de la forma:
$$
...
*obs:* una curva $\Gamma$ se dice [[Curva Seccionalmente Suave|seccionalmente suave]] si $\Gamma=\Gamma_{1}+\Gamma_{2}+\dots+\Gamma_{n}$, donde $\Gamma_{i}$ son suaves. O sea una curva es seccionalmente suave si se puede escribir como unión de curvas suaves.
...
\end{align}
$$
![[Pasted image 20250828112530.png#invert_B]]
...
y $\vec{\gamma}(t_{0})\neq 0$ es un vector tangente a la curva. 
* Es la recta tangente común que pasa por un punto.

Esto dice que $\Gamma$ puede representar una trayectoria recorrida por una partícula en el instante $t$, mediante $\vec{\gamma}(t)$, y el vector $\vec{\gamma}'(t)$ representa la velocidad de la partícula en instante 't'.
...
*obs:* Una curva puede ser recorrida en 2 sentidos opuestos, por ejemplo, si se tiene una parametrización $\vec{\gamma}:[a,b] \to \mathbb{R}^{n}$ que recorre la curva según:
$$
...
*Ejemplo*: La parametrización del segmento de recta que une los puntos $x_{0},x_{1}\in \mathbb{R}^{n}$ está dada por: $\vec{\gamma}(t)=(1-t)x_{0}+tx_{1},\; t\in[0,1]$,
y la curva en el sentido opuesto **para este caso** es: $\tilde{\vec{\gamma}}(t)=tx_{0}+(1-t)x_{1},\; t\in[0,1]$

# Longitud de Curva
Dada una curva $\Gamma$ parametrizada por $\vec{\gamma}:[a,b] \to \mathbb{R}^{n}$, y sea $P=\{ t_{0}=a,t_{1},t_{2},\dots ,t_{n}=b \}$  una partición de $[a,b]$, se define la **longitud de la curva:**
$$
L_{p}(\Gamma)=\sum_{i=1}^{n}\lvert\lvert \vec{\gamma(t_{i})}-\vec{\gamma}(t_{{i-1}}) \rvert\rvert 
$$
![[Pasted image 20250828112711.png#invert_B]]
- Cuando se agregan puntos, se acerca a la longitud de la curva completa.

Se dice que $\Gamma$ es **rectificable** si el conjunto $\{ L_{p}(\Gamma),P \text{ es una particion de }[a,b] \}$ es acotado superiormente.

\**recuerdo:* Si el conjunto es acotado superiormente, entonces tiene supremo. Además, tiene ínfimo por la norma ser no negativa.
Luego, la longitud de la curva (o del arco) se define como:
$$
L(\Gamma)=\text{sup}\{ L_{p}(\Gamma),\;P\text{ es una particion de }[a,b] \}
$$

Y para tener la longitud de la curva $L_{P}(\Gamma)$ sin calcular supremo: 
...
#### Proposición
Si $\Gamma = \Gamma_{1}+\dots+\Gamma_{k}$ es **rectificable** $\iff \Gamma_{i},\;i\in \{ 1,\dots,k \}$ son rectificables, y en ese caso:
$$
L_{P}(\Gamma)=L(\Gamma_{1})+L(\Gamma_{2})+\dots+L(\Gamma_{k})
$$

#### Teorema
Si $\Gamma$ es una [[Curva suave|curva suave]] parametrizada por $\vec{\gamma}:[a,b] \to \mathbb{R}^{n}$, entonces $\Gamma$ es rectificable y se cumple que:
$$
L(\Gamma)=\int_{a}^{b} ||\vec{\gamma}'(t)||dt
$$
> Si curva la es suave, entonces se puede tener longitud
...
$$
\vec{\gamma}(t)=\left( \cos t,\sin t,t \right) ,0\leq t\leq 2\pi
$$
está dada por:
$$
\begin{align}
L(\Gamma) & =\int_{0}^{2\pi} \lvert \lvert (-\sin t,\cos t,1) \rvert  \rvert  \, dt  \\
 & =\int_{0}^{2\pi} 2 \, dt \\
 & =2\pi \sqrt{ 2 }
\end{align}
$$
...
* Si $\Gamma$ es un desplazamiento rectilíneo dado por un vector $\vec{d}$, y $\vec{F}$ es constante, entonces el trabajo realizado $\mathbb{W}=\vec{F}\cdot\vec{d}$
...
![[Pasted image 20250828113241.png#invert_B]]

luego, parametrizando la curva $\Gamma$ por $\vec{\gamma}:[a,b]\to \mathbb{R}^{n}$, si $f$ varía en un intervalo pequeño de tiempo $[t_{i},t_{i}+\Delta t]$, la partícula se moverá entre $\vec{\gamma}(t_{i})$ y $\vec{\gamma}(t_{i}+\Delta t)$ en un **desplazamiento vectorial** dado por:
$$
\Delta  \vec{d}=\vec{\gamma}(t_{i}+\Delta t)-\vec{\gamma}(t_{i})
$$
y multiplicando por un 1 conveniente: $\frac{\Delta t}{\Delta t}$
$$
\Delta  \vec{d}= \Delta t\cdot \frac{\vec{\gamma}(t_{i}+\Delta t)-\vec{\gamma}(t_{i})}{\Delta t}
$$
en donde el término $\frac{\vec{\gamma}(t_{i}+\Delta t)-\vec{\gamma}(t_{i})}{\Delta t}$ es similar a una derivada.
Ahora, por definición de derivada:
$$
...
$$
\vec{F}(\vec{\gamma}(t_{i}))\cdot\Delta  \vec{d} \approx  \vec{F}(\vec{\gamma}(t_{i}))·\vec{\gamma}(t_{i})\Delta t
$$
...
***def:*** Dada $\Gamma$ curva suave en $\mathbb{R}^n$ y $\vec{F}$ un campo vectorial continua y definido en $\text{tr}(\Gamma)$, entonces la **integral de línea** de $\vec{F}$ a lo largo de $\Gamma$ está definida como:
$$
...
*Ejemplo:* Si $n=2$, usualmente se escribe:
...
### Teorema:
Si $\Gamma$ es una cuerva seccionalmente suave y $\vec{F}$ un campo vectorial definido y continuo sobre $\text{tr}(\Gamma)$, entonces se cumple que:
$$
...
## Caso con campo escalar:
Si $f$ es un campo escalar, se define la integral de línea de $f$ sobre $\Gamma$ como:
$$
...
Dada $\vec{\alpha}:[a,b] \to \mathbb{R}^{n}$ una parametrización de $\Gamma$, la cual es una [[Curva cerrada|curva cerrada]]. 
Sean $a<c<b$, y $\vec{\beta}:[c,c+b-a]\to \mathbb{R}^{n}$ definida por:
...
***
Si $\vec{F}$ es un campo vectorial continuamente definido en $\text{tr}(\Gamma)$ y $\Gamma$ es suave seccionalmente, entonces:
$$
\begin{align}
...
*obs 1*: La integral de línea sobre una curva cerrada se suele denotar por:
$$
\oint_{\Gamma}=\vec{F} d\vec{x}
$$
*obs 2*: la que sí hace cambiar el valor de la integral a lo largo de una curva cerrada $\Gamma$, es cuando se cambia el **número de veces que se recorre la curva y cuando se cambia el sentido de recorrido.**

# Integrales de línea independientes del camino (o curva)
Si se considera la situación de tener dos curvas distintas pero que ambas unen los puntos $(0,0)$ con $(1,1)$ en donde:
$\Gamma_{1}$: La parte de la parábola $y=x^{2}$
$\Gamma_{2}$: El segmento de recta que va desde $(0,0)\to(1,1)$
![[Pasted image 20250814112231.png#invert_B]]
...
$$
\begin{align}
 & \Gamma_{1}:\vec{\gamma}_{1}(t)=(t,t^{2}),0\leq t\leq 1 \\
 & \Gamma_{2}:\vec{\gamma}_{2}(t)=(t,t),0\leq t\leq 1 \\
\int_{\Gamma_{1}} & =\int_{0}^{1} F(t,t^{2})(1,2t) \, dt  =\int_{0}^{1} (t+t^{4},t^{2})(1,2t) \, dt =\int_{0}^{1} (t+t^{4}+2t^{3}) \, dt \\
 & =\left( \frac{t^{2}}{2} + \frac{t^{5}}{5}+\frac{2}{4}t^{4}\right) |_{0}^{1} =\frac{6}{5}   \\
\int_{\Gamma_{2}} & =\int_{0}^{1} \vec{F}(t,t)(1,1) \, dt =\int_{0}^{1} (t+t^{2},t^{2})(1,1) \, dt =\int_{0}^{1} (t+2t^{2}) \, dt \\
 & =\left( \frac{t^{2}}{2}+\frac{2}{3}t^{3} \right)|_{0}^{1} =\frac{7}{5}\neq \frac{6}{5}  \\
\vdots  \\
 & \text{innecesario}
\end{align}
$$

***def:*** Si $\vec{F}$ es un campo vectorial continuo en un abierto $\Omega \subseteq \mathbb{R}^{n}$ y $\int_{\Gamma_{1}}\vec{F}d\vec{x}=\int_{\Gamma_{2}}\vec{F}d\vec{x}$ para todo par de curvas $\Gamma_{1},\Gamma_{2}\subseteq \Omega$ con el mismo punto inicial y final, entonces se dice que **las integrales de línea son independientes del camino** en $\Omega$

*obs:* Hay casos en que lo anterior se denota por:
$$
\int_{x_{0}}^{x_{1}} \vec{F} d\vec{x}
$$
para cualquier curva seccionalmente suave desde $x_{0}$ a $x_{1}$

### Teorema
Si se tiene un campo vectorial $\vec{F}$ continuo en un conjunto abierto $\Omega$. Entonces,
...
*def:* Dado $\vec{F}:\Omega \subseteq \mathbb{R}^{n}\to \mathbb{R}^{n}$ se dice **campo conservativo** si existe un campo escalar diferenciable $f:\Omega \subseteq \mathbb{R}^{n}\to \mathbb{R}$ tal que:
$$
...
*def:* Dado conjunto $\Omega \subseteq \mathbb{R}^{n}$ abierto, $\Omega$ es un **conjunto conexo** si todos los puntos de $\Omega$ pueden unirse mediante una curva seccionalmente suave en $\Omega$.
Gráficamente:
![[Pasted image 20250815192136.png#invert_B]]
...
*Obs 1:* si $\vec{F}=\nabla f$ se cumple que cualquier curva seccionalmente suave, con punto inicial $x_{0}$ y final $x_{1}$:
  ...
  $$
  O sea si se tiene un campo vectorial conservativo, no es necesario saber la parametrización. Sólo importa el valor del potencial en los extremos.

*Obs 2:* Si se tiene un campo vectorial $\vec{F}:\Omega \subseteq \mathbb{R}^{n}\to \mathbb{R}^{n}$ continuo y $\Omega$ es conexo, entonces son equivalentes:
...
$$
*Obs:* Para tres dimensiones, osea si $N=3$: $\vec{F}(x,y,z)=(P(x,y,z),Q(x,y,z),R(x,y,z))$, el resultado anterior se reduce a:
  $$
  ...
  $$
para este caso existe una forma equivalente al teorema anterior, en términos del rotacional o rotor de $\vec{F}$

## Rotacional o Rotor
*def:* Dado un campo vectorial $\vec{F}=(P,Q,R)$ definido en un conjunto abierto conexo $\Omega \subseteq \mathbb{R}^{3}$, se define el **rotacional o rotor** de $\vec{F}$, denotado como $\text{rot}(\vec{F})$, como un campo vectorial $\text{rot}(\vec{F}):\Omega \subseteq \mathbb{R}^{3}\to \mathbb{R}^{3}$, de la forma:
$$
...
$$
que es más fácil de recordar si se escribe de la forma:
$$
\text{rot}(\vec{F})=\det
\left( \begin{matrix}
...
*Obs:* Intuitivamente, un dominio estrellado no tiene 'agujeros'. Graficamente:
...
## Encontrando Potencial de Campo Vectorial Conservativo
