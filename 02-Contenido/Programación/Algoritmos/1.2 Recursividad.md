Las funciones recursivas son las cuales se llaman a sí mismas. En donde se calcula una cierta instancia de un problema en base a una menor. Para resolver el problema de un tamaño $n$, se debe primero resolver el mismo problema para un tamaño $n-1$ y así sucesivamente hasta llegar a un **caso base**.
## Ejemplos
### Ejemplo: Exponenciación
Una exponenciación $y = x^{n}$  se puede definir de la forma:
$$
y = x^{n} = 
\left\{
	\begin{array}{ll}
...
\right.
$$
que se implementa recursivamente de la forma:
```python
def exp(x,n):
	if (n == 0):
		return 1
...
```
en la línea `exp(x, n-1)` se hace el llamado a la misma función, pero con un menor valor del exponente $n$.
...
Otra implementación era con el [[1.1 Propiedad Invariante#Exponenciación (Algoritmo Binario)|algoritmo binario]], en donde se hace $x^{k}=(x^{2})^{\frac{k}{2}}$ tratando aparte el caso $n$ par:
...
def exp_recursivo(x,n):
	...
	if (n == 0):
		...
		return x*exp(x,n-1)
```
esta versión recursiva demora tiempo $O(\log n)$.

### Ejemplo: Buscar Máximo con Recursividad
...
Implementación: Se pide que se entregue todos los posibles números binarios que se pueden escribir con una cierta cantidad de dígitos.
